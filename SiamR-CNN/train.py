...
class ResNetC4Model(DetectionModel):
    def inputs(self):
        ret = [
            tf.compat.v1.placeholder(tf.float32, (None, None, 3), 'image'),
            tf.compat.v1.placeholder(tf.int32, (None, None, cfg.RPN.NUM_ANCHOR), 'anchor_labels'),
            tf.compat.v1.placeholder(tf.float32, (None, None, cfg.RPN.NUM_ANCHOR, 4), 'anchor_boxes'),
            tf.compat.v1.placeholder(tf.float32, (None, 4), 'gt_boxes'),
            tf.compat.v1.placeholder(tf.int64, (None,), 'gt_labels')]  # all > 0
        if cfg.MODE_MASK:
            ret.append(
                tf.compat.v1.placeholder(tf.uint8, (None, None, None), 'gt_masks')
...
class ResNetFPNModel(DetectionModel):
    ...
    def inputs(self):
        ...
        for k in range(len(cfg.FPN.ANCHOR_STRIDES)):
            ret.extend([
                tf.compat.v1.placeholder(tf.int32, (None, None, num_anchors),
                               'anchor_labels_lvl{}'.format(k + 2)),
                tf.compat.v1.placeholder(tf.float32, (None, None, num_anchors, 4),
        ...
        if cfg.EXTRACT_GT_FEATURES:
            ret.append(tf.compat.v1.placeholder(tf.float32, (None, 4,), 'roi_boxes'))
        return ret
...
class ResNetFPNTrackModel(ResNetFPNModel):
    def inputs(self):
        ...
        if cfg.USE_PRECOMPUTED_REF_FEATURES:
            ret.append(tf.compat.v1.placeholder(tf.float32, (256, 7, 7), 'ref_features'))
        else:
            ret.append(tf.compat.v1.placeholder(tf.float32, (None, None, 3), 'ref_image'))
            ret.append(tf.compat.v1.placeholder(tf.float32, (4,), 'ref_box'))
        if cfg.MODE_THIRD_STAGE:
            ret.append(tf.compat.v1.placeholder(tf.float32, (256, 7, 7), 'ff_gt_tracklet_feat'))
            ret.append(tf.compat.v1.placeholder(tf.float32, (None, 256, 7, 7), 'active_tracklets_feats'))
            ret.append(tf.compat.v1.placeholder(tf.float32, (None, 4), 'active_tracklets_boxes'))
            ret.append(tf.compat.v1.placeholder(tf.float32, (), 'tracklet_distance_threshold'))
        if cfg.MODE_HARD_MINING:
            ret.append(tf.compat.v1.placeholder(tf.float32, (None, 3, 256, 7, 7), 'hard_negative_features'))
            if cfg.MODE_IF_HARD_MINING_THEN_ALSO_POSITIVES:
                ret.append(tf.compat.v1.placeholder(tf.float32, (None, 3, 256, 7, 7), 'hard_positive_features'))
                ret.append(tf.compat.v1.placeholder(tf.float32, (None, 3), 'hard_positive_ious'))
                ret.append(tf.compat.v1.placeholder(tf.float32, (None, 4), 'hard_positive_gt_boxes'))
                ret.append(tf.compat.v1.placeholder(tf.float32, (None, 3, 4), 'hard_positive_jitter_boxes'))
        if cfg.EXTRACT_GT_FEATURES:
            ret.append(tf.compat.v1.placeholder(tf.float32, (None, 4,), 'roi_boxes'))
        return ret
